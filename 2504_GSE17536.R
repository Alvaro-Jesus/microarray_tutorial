#--------------------------------------------------------
# Title: Microarray Data Analysis with GEOquery and limma for one color


#

#--------------------------------------------------------
# Part 1: loading packages and functions
#--------------------------------------------------------
pacman::p_load(
    GEOquery, tidyverse, ggrepel, limma, oligo, DT, pheatmap, tidyplots, affy, oligoClasses, testit
)

# function is log2transformed
isLog2Transformed <- function(data) {
    qx <- as.numeric(quantile(data, c(0., 0.25, 0.5, 0.75, 0.99, 1.0), na.rm = T))
    shouldBeLogged <- (qx[5] > 100) || (qx[6] - qx[1] > 50 && qx[2] > 0)
    return(!shouldBeLogged)
}

#--------------------------------------------------------
# Part 2: Obtaining metadata and raw data
#--------------------------------------------------------
# First obtain metadata
id <- "GSE17536" # GPL4133

# creating a temp folder
if (!dir.exists(paste0(".temp/", id))) {
    dir.create(paste0(".temp/", id))
} else {
    message("Folder already exists!")
}
# Obtaining Metadata
meta <- getGEO(id, GSEMatrix = TRUE, destdir = ".temp")
meta <- meta[[1]]


head(fData(meta)) #genes
colnames(fData(meta))
head(pData(meta)) #clinic
colnames(pData(meta))
table(pData(meta)[,54])
head(exprs(meta)) #matrix gene expression
colnames(exprs(meta))


tail(pData(meta), 5)
dim(meta)

# Metadata wrangling
tail(pData(meta)$"age:ch1", 5)
names(pData(meta))
pd <- pData(meta) |>
    tibble::rownames_to_column("ID") |>
    dplyr::select("age:ch1", "gender:ch1", "overall survival follow-up time:ch1", "overall_event (death from any cause):ch1", ID, supplementary_file, title) |>
    dplyr::mutate(file = str_split(supplementary_file, "/") |> map_chr(tail, 1)) |>
    dplyr::rename(age = "age:ch1", gender = "gender:ch1", OS_event= "overall_event (death from any cause):ch1", OS_months="overall survival follow-up time:ch1")  |>
    mutate(comparison = case_when(
        str_detect(OS_event, "n") ~ "live",
        str_detect(OS_event, "d") ~ "death",
        .default = NA)) 


pd$OS_event
table(pd$comparison)

pd$comparison


#para aumentar el tiempo de respeusta de descarga
options(timeout = max(300, getOption("timeout")))
options(download.file.method.GEOquery = "curl")


## Download all the files in the .temp of this environment
for (i in 1:length(pd$supplementary_file)) {
    url <- pd$supplementary_file[i]
    destfile <- file.path(paste0(".temp/", id, "/", pd$file[i]))
    # Download the file
    tryCatch(
        {
            download.file(url, destfile, mode = "wb",method="curl") ### variar de protocolos
        },
        error = function(e) {
            # Fallback to default method if curl fails
            download.file(url, destfile, mode = "wb") ###
        }
    )
    # Optional: Extract if it's a tar file
    if (grepl("\\.tar(\\.gz)?$", destfile)) {
        untar(destfile, exdir = paste0(".temp/", id))
    }
}

#--------------------------------------------------------


#--------------------------------------------------------

# Subsettting files 
table(pd$comparison)

#### gene Annotation
gpl2 <- getGEO("GPL570")
head(Table(gpl2))

annot <- Table(gpl2)[,c("ID", "GB_ACC", "Gene Symbol", "Gene Title", "ENTREZ_GENE_ID")] #|>
    tibble::column_to_rownames("ID")


head(annot)
### Read the raw files

affy_data = ReadAffy(
    filenames=pd$file,
    sampleNames = pd$ID,
    celfile.path = file.path(".temp", id)
)




## QC pre normalización?

if (!require("BiocManager", quietly = TRUE))
    install.packages("BiocManager")
BiocManager::install("affyQCReport")  # Instalar desde Bioconductor

BiocManager::install("affyQCReport")

affyQCReport(affy_data, destdir = "QC_AffyReport")


## Normalisation?
affy_rma = rma(affy_data)
methods(class=class(affy_data))
protocolData(affy_data)


head(exprs(affy_rma))
getwd()

## QC con html
pacman::p_load(arrayQualityMetrics)
arrayQualityMetrics(affy_rma, outdir = "QC_Report")







#--------------------------------------------------------
# Part 6: DE
#--------------------------------------------------------
# Create design matrix based on final_subtype
design <- model.matrix(~ 0 + factor(pd$comparison))
rownames(design) <- pd$ID
colnames(design) <- levels(factor(pd$comparison))
head(design, 20)

# Make contrasts (adjust based on your comparisons)
levels(factor(pd$comparison))
contrast.matrix <- makeContrasts(
    d_l = death - live ,
    levels = design
)
fit2 <- lmFit(affy_rma, design) %>%
    contrasts.fit(contrast.matrix) %>%
    eBayes()

top_probes <- topTable(fit2, number = Inf, adjust.method = "BH" ,  p.value = 0.05)
str(top_probes)
head(top_probes)

colnames(annot)[1]
annot[,1]
rownames(top_probes)
top_probes= rownames_to_column(top_probes, var="ID")
final=merge(top_probes,annot, by="ID")
dim(final)
final[1:50,]
#QC


