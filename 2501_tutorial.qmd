...
gpl_annot |>
  filter(str_detect(CHROMOSOMAL_LOCATION, "chrX")) |>
  separate(
  CHROMOSOMAL_LOCATION,
  into = c("CHROMOSOME", "LOCATION"),  # New column names
  sep = ":"
) |>
  separate(
  LOCATION,
 ...
 mutate(
    genint = ifelse(
      between(as.numeric(START), 117957753, 117960931) & 
      between(as.numeric(END), 117957753, 117960931),
      "YES", 
      "NO"
    )
  ) |>
  # Filter for probes fully within the interval
  filter(genint == "YES")

zinc_finger_probes <- gpl_annot[gpl_annot$CHROMOSOME == "20" & 
                                gpl_annot$START >= 61678917 & 
                                gpl_annot$END <= 61682078, ]
zinc_finger_probes <- gpl_annot[grep("ZCCHC12|CCHC.*12|54800", 
                                    gpl_annot$GENE_SYMBOL, 
                                    ignore.case = TRUE), ]
zinc_finger_probes
gpl2 = getGEO("GPL10332")
annot <- Table(gpl2)[, c("ID", "GENE_SYMBOL", "ENSEMBL_ID")]
annot |>
