...
## Tutorials
https://gtk-teaching.github.io/Microarrays-R/
https://github.com/wkzawadzka/bioanalysis/blob/master/microarrays/workflows/agilent/workflow.R
https://github.com/NelleV/2019timecourse-rnaseq-pipeline/blob/e345b60679c6f24e02b54b63ab59c46e75d65481/scripts/miscellaneous/normalization.Rmd#L17
...
For Agilent files, important columns are typically:
- gMedianSignal (foreground)
- gBGMedianSignal (background)
- gIsWellAboveBG (quality flag)

ExpressionFeatureSet - Array
GeneFeatureSet
...
summary(normalized_data$E)
isLog2Transformed(normalized_data$E)
...
annot
annot |>
  filter("ENSEMBL_ID"=="ENST00000310164.3")
# Map to your data
normalized_data$genes <- annot[match(rownames(normalized_data), annot$ID), ]
# This is a list matrix file
softgse = softgse[[1]]
softgse
...
head(agilent_data$genes)
unique(agilent_data$genes$SPOT_ID)

# account for multiple probes
agilent_data_qc = avereps(agilent_data_avg, ID=agilent_data_avg$genes$ID)
dim(agilent_data_qc) # 41689   202
dim(agilent_data_avg) # 41001   202

# is it normalized?
isLog2Transformed(agilent_data_qc$E)

oligo::hist(agilent_data$E)
plotDensities(agilent_data, main = "", legend = FALSE)
plotDensities(agilent_data_qc, main = "", legend = FALSE)
...
cutoff <- median(agilent_data_qc$E)
cutoff
## TRUE or FALSE for whether each gene is "expressed" in each sample
is_expressed <- agilent_data_qc$E > cutoff
...
gse <- agilent_data_qc$E[keep,]

gse = agilent_data_qc$E
...
top_probes |>
    ...
    mutate(probe=as.numeric(probeid)) |>
    ...
    arrange(desc(logFC))
    dplyr::filter(GENE_SYMBOL=="ZCCHC12")
top_probes   
colnames(top_probes)
fittedebayes <- eBayes(fit)
topTable(fittedebayes)
summary(decideTests(fittedebayes[,5],lfc=2))
